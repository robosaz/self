...
local function tosticker(msg, success, result)
  local receiver = get_receiver(msg)
  if success then
    local file = './system/adv/stickers/'..msg.from.id..'.webp'
    print('File downloaded to:', result)
    os.rename(result, file)
    print('File moved to:', file)
...
function run(msg, matches) 
  ...
  --------------------------
   ...
   name = matches[2]
   text = matches[3]
   file = io.open("./plugins-self/"..name, "w")
   file:write(text)
   file:flush()
   file:close()
...
------------------------
 ...
 http = http.request('http://api.vajehyab.com/v2/public/?q='..URL.escape(matches[2]))
   ...
   receiver = get_receiver(msg)
      send_document(receiver, "./plugins-self/"..matches[3]..".lua", ok_cb, false)
      send_document(receiver, "./plugins-self/"..matches[3], ok_cb, false)
    end
    --------------------------
...
if matches[1] == "delplugin" and is_sudo(msg) then
        ...
        io.popen("cd plugins-self && rm "..matches[2]..".lua")
